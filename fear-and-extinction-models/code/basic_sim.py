...
# ----------------------------------------------------------------------------
# PTSD + DBS (Implementation 1) Fear Extinction (Protocol 2)
# During fear association stages, closed loop DBS
# NOT helpful, not how it works exactly
# ----------------------------------------------------------------------------
    ...
    def fear_stages_simulations(l):
        ...
        start_scope()
        net, neurons, conn, Pe, Pi, spikemon, statemon, PG_cs, PG_ctx_A, PG_ctx_B, CS_e, CS_i, CTX_A, CTX_B = \
            amygdala_net(input=True, input_vars=new_input_vars,pcon=pcon, wsyn=wsyn, sdel=sdelay,PTSD=True,DBS=1,record_weights=True)
        # Run the full protocol
        net.run(tsim*ms, report='stdout')
...
# ----------------------------------------------------------------------------
# PTSD + DBS (Implementation 1) Fear Extinction (Protocol 2)
# During fear extinction stages
# Increases learning rate because of chemical feedback to 
# ----------------------------------------------------------------------------
    ...
    def fear_stages_simulations(l):
        ...
        start_scope()
        net, neurons, conn, Pe, Pi, spikemon, statemon, PG_cs, PG_ctx_A, PG_ctx_B, CS_e, CS_i, CTX_A, CTX_B = \
            amygdala_net(input=True, input_vars=new_input_vars,pcon=pcon, wsyn=wsyn, sdel=sdelay,PTSD=True,DBS=2,record_weights=True)
        # Run the full protocol
        net.run(tsim*ms, report='stdout')
